...
This reference gateway pins OpenClaw `2026.7.1-2` and Node `22.22.3`. Its nine typed `brain_*` tools call the shared core; it has no second wiki or knowledge database.
...
Open `http://127.0.0.1:18789`. The host port is loopback-only, while the gateway binds `lan` inside its isolated Docker network so the published port works. The health check uses `/healthz`.
